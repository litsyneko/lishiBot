  ...
  async ready() {
    ...
    try {
      ...
      const commands = await rest.get(
        Routes.applicationGuildCommands(this.client.user!.id, config.guilds[0]),
      ) as Array<{ id: string; name: string }>

      const serverCommand = commands.find((c) => c.name === '서버')
      if (serverCommand !== undefined) {
        await rest.patch(
          Routes.applicationGuildCommand(this.client.user!.id, config.guilds[0], serverCommand.id),
          {
            body: {
        ...
        logger.info('Command', '/서버 명령어 권한을 서버 관리 이상으로 제한했어요.')
      }
    } catch (err) {
      logger.error('Command', `명령어 권한 설정 실패: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
