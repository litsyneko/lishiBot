  ...
  async raw(payload: unknown) {
    try {
      ...
      if (data.t === 'VOICE_CHANNEL_EFFECT_SEND') {
        logger.info('SoundboardGuard', `이벤트 수신: guild=${data.d?.guild_id} user=${data.d?.user_id} sound=${data.d?.sound_id}`)
      }
      ...
      if (guildId === undefined || userId === undefined || soundId === undefined) return

      logger.info('SoundboardGuard', `활성화 상태: guild=${guildId} enabled=${isSoundboardGuardEnabled(guildId)}`)
      ...
      if (entry !== undefined && now < entry.mutedUntil) {
        logger.info('SoundboardGuard', `${member.displayName}: 이미 제한 중 (${entry.count}회, mute 유지)`)
        return
      }

      if (entry === undefined || now - entry.firstAt > WINDOW_MS) {
        this.counters.set(key, { count: 1, firstAt: now, mutedUntil: 0 })
        logger.info('SoundboardGuard', `${member.displayName}: 카운터 시작/리셋`)
        return
      }

      entry.count += 1
      logger.info('SoundboardGuard', `${member.displayName}: ${entry.count}/${SOUND_LIMIT} (윈도우 ${(now - entry.firstAt) / 1000 | 0}초)`)
      ...
      if (entry.count > SOUND_LIMIT) {
        ...
        try {
          ...
          logger.info('SoundboardGuard', `${guild.name} - ${member.displayName}: 사운드보드 ${entry.count}회 사용 → 30초 사운드보드+음소거 제한`)
          ...
          setTimeout(async () => {
            try {
              ...
              if (ch !== undefined && ch.isVoiceBased()) {
                await ch.permissionOverwrites.delete(member.id, '사운드보드 제한 해제')
              }
              const freshMember = await guild.members.fetch(userId).catch(() => undefined)
              if (freshMember !== undefined && freshMember.voice.channel !== null) {
                await freshMember.voice.setMute(false, '사운드보드 제한 해제').catch(() => {})
              }
            } catch (err) {
              logger.error('SoundboardGuard', `${guild.name} - ${member.displayName}: 제한 해제 실패 - ${err instanceof Error ? err.message : String(err)}`)
            }
            this.counters.delete(key)
          }, TIMEOUT_MS)
        } catch (err) {
          logger.error('SoundboardGuard', `${guild.name} - ${member.displayName}: 제한 적용 실패 - ${err instanceof Error ? err.message : String(err)}`)
        }
      }
    } catch (err) {
      logger.error('SoundboardGuard', `이벤트 처리 오류 - ${err instanceof Error ? err.message : String(err)}`)
    }
  }
}
