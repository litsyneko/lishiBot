import type { ToolDefinition, ToolExecutionContext, PermissionCheckResult } from '../tools/toolTypes'
...
export function checkToolPermissionLayer3(
  ...
  if (definition.permission.requireAdmin && !hasAdmin) {
    return { ok: false, reason: '권한이 변경되어 이 작업을 더 이상 실행할 수 없어요. (관리자 권한 필요)' }
  }
  if (definition.permission.requireManageGuild && !hasManageGuild) {
    return {
      ok: false,
      reason: '권한이 변경되어 이 작업을 더 이상 실행할 수 없어요. (서버 관리 권한 필요)',
    }
  }

  // L3b: runtime check (dynamic conditions like voice channel membership)
  if (definition.permission.runtimeCheck !== undefined) {
    const runtimeError = definition.permission.runtimeCheck(args, context.guildId, context.userId)
